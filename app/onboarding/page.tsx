...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (data.user.user_metadata?.tenant_name) {
        ...
        setTenantName(`${data.user.user_metadata.full_name} İşletmesi`);
      }
    };
    checkUser();
  }, [supabase]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!tenantName.trim()) {
      toast.error("Lütfen işletme adınızı girin.");
      return;
    }
    ...
    try {
      ...
      toast.error("İşletme kurulurken bir hata oluştu: " + (error.message || "Bilinmeyen hata"));
      setIsLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md relative z-10">
        <Card className="rounded-3xl shadow-2xl border-white/10 bg-white/95 backdrop-blur-xl">
          <CardHeader className="text-center pb-2">
            ...
            <CardTitle className="text-2xl font-extrabold">Son Bir Adım!</CardTitle>
            <CardDescription>Size özel alanınızı oluşturmak için işletme (Etüt Merkezi vb.) adınızı belirleyin.</CardDescription>
          </CardHeader>
          <CardContent className="pt-6">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="tenantName" className="font-semibold text-slate-700">İşletme Adınız</Label>
              ...
              <Button type="submit" disabled={isLoading} className="w-full py-6 rounded-xl text-md font-bold shadow-lg hover:shadow-xl transition-all">
                {isLoading ? "Kuruluyor..." : "İşletmemi Kur ve Başla"}
